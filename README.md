<!-- markdownlint-disable MD024 -->
# 📓 Marchi Notes / Anotações do Marchi

## 🔎 Index / Índice

- [📓 Marchi Notes / Anotações do Marchi](#-marchi-notes--anotações-do-marchi)
  - [🔎 Index / Índice](#-index--índice)
  - [:us: English Version](#us-english-version)
    - [👋 About](#-about)
    - [👑 Most Useful Resources](#-most-useful-resources)
    - [🗂️ Structure](#️-structure)
      - [🗃️ Database](#️-database)
      - [👨‍💻 Development](#-development)
      - [🛠️ DevOps](#️-devops)
      - [🌳 Environment](#-environment)
      - [🎮 Game Design](#-game-design)
      - [🖥️ Systems Analysis and Development (SAD)](#️-systems-analysis-and-development-sad)
      - [🌐 Web Basics](#-web-basics)
      - [🎨 Web Design](#-web-design)
  - [:brazil: Versão em Português](#brazil-versão-em-português)
    - [👋 Sobre](#-sobre)
    - [👑 Recursos mais úteis](#-recursos-mais-úteis)
    - [🗂️ Estrutura](#️-estrutura)
      - [🗃️ Banco de Dados](#️-banco-de-dados)
      - [👨‍💻 Desenvolvimento](#-desenvolvimento)
      - [🛠️ DevOps](#️-devops-1)
      - [🌳 Ambiente](#-ambiente)
      - [🎮 Design de Jogos](#-design-de-jogos)
      - [🖥️ Análise e Desenvolvimento de Sistemas (ADS)](#️-análise-e-desenvolvimento-de-sistemas-ads)
      - [🌐 Básico de Web](#-básico-de-web)
      - [🎨 Design de Web](#-design-de-web)

## :us: English Version

### 👋 About

- This is a repository where I take notes about things I learn, to help me remember things.
- It's also a way to force me to learn more about content I already know.
- I might also write down some tips and tricks I find useful, that could help others.
- Some things might be in english, some in Brazilian portuguese, and some in both.
- If you want to use it, feel free to do so. If you find any mistakes, or improvements, please let me know.
- If you find it useful in any way, consider leaving a star for the repository! 😄

### 👑 Most Useful Resources

Things I use on a daily basis and tips for other programmers:

- [Conventional Commits](environment/git/conventional_commits.md)
- [JavaScript](development/javascript/javascript.md)
- [VSCode extensions](environment/vscode/vscode_extensions.md)

### 🗂️ Structure

Main folders:

- [Database](database)
- [Development](development)
- [DevOps](devops)
- [Environment](environment)
- [Game Design](game_design)
- [Systems and Analysis Development (SAD)](systems_analysis_&_development)
- [Web Basics](web_basics)
- [Web Design](web_design)

#### 🗃️ Database

Database related content, including JavaScript ORMs:

- [Database](database)
  - [MongoDB](database/mongodb)
  - [SQL](database/sql)

#### 👨‍💻 Development

Programming languages and its frameworks:

- [Development](development)
  - [C#](development/c#)
  - [C++](development/c++)
  - [JavaScript](development/javascript)
  - [Python](development/python)
  - [TypeScript](development/typescript)

#### 🛠️ DevOps

DevOps related content:

- [DevOps](devops)
  - [Docker](devops/docker)
  - [Heroku](devops/heroku)

#### 🌳 Environment

Environment related content, with general or OS specific content:

- [Environment](environment)
  - [Git](environment/git)
  - [Unix](environment/unix)
  - [VSCode](environment/vscode)

#### 🎮 Game Design

Game Design related content:

- [Game Design](game_design)
  - [Life & Career](game_design/life_&_career)
  - [Visual Expression](game_design/visual_expression)

#### 🖥️ Systems Analysis and Development (SAD)

My associate's degree in Systems and Analysis Development:

- [Systems and Analysis Development (SAD)](systems_analysis_&_development)
  - [Web Development](systems_analysis_&_development/web_development)

#### 🌐 Web Basics

Web basics or generic content that can be used in any web project:

- [Web Basics](web_basics)
  - [HTML](web_basics/html)
  - [CSS](web_basics/css)
  - [Regex](web_basics/regex)

#### 🎨 Web Design

Web Design related content:

- [Web Design](web_design)
  - [Creating a Design](web_design/creating_a_design)

## :brazil: Versão em Português

### 👋 Sobre

- Este é um repositório onde eu anoto coisas que aprendo, para me ajudar a lembrar dos conteúdos
- Também é uma forma de me forçar a aprender mais sobre coisas que eu já conheço.
- Também posso usar para escrever algumas dicas e truques que eu acho úteis, que podem ajudar outras pessoas.
- Alguns conteúdos podem estar em inglês, outros em português, e alguns em ambos.
- Se você quiser usar, fique à vontade. Caso encontre algum erro ou melhoria, por favor me avise.
- Se achar útil de alguma maneira, considere deixar uma estrela para o repositório! 😄

### 👑 Recursos mais úteis

Coisas que eu uso no dia a dia e dicas para outros programadores:

- [Padrões para Commits](environment/git/conventional_commits.md)
- [JavaScript](development/javascript/javascript.md)
- [Extensões do VSCode](environment/vscode/vscode_extensions.md)

### 🗂️ Estrutura

Pastas principais:

- [Banco de Dados](database)
- [Desenvolvimento](development)
- [DevOps](devops)
- [Ambiente](environment)
- [Game Design](game_design)
- [Análise e Desenvolvimento de Sistemas (ADS)](systems_analysis_&_development)
- [Web Básica](web_basics)

#### 🗃️ Banco de Dados

Conteúdo relacionado a banco de dados, incluindo ORMs em JavaScript:

- [Banco de Dados](database)
  - [MongoDB](database/mongodb)
  - [SQL](database/sql)

#### 👨‍💻 Desenvolvimento

Linguagens de programação e seus frameworks:

- [Desenvolvimento](development)
  - [C#](development/c#)
  - [C++](development/c++)
  - [JavaScript](development/javascript)
  - [Python](development/python)
  - [TypeScript](development/typescript)

#### 🛠️ DevOps

Conteúdo relacionado a DevOps:

- [DevOps](devops)
  - [Docker](devops/docker)
  - [Heroku](devops/heroku)

#### 🌳 Ambiente

Conteúdo relacionado ao ambiente, com conteúdo geral ou específico de um sistema operacional:

- [Ambiente](environment)
  - [Git](environment/git)
  - [Unix](environment/unix)
  - [VSCode](environment/vscode)

#### 🎮 Design de Jogos

Conteúdo relacionado a Game Design:

- [Design de Jogos](game_design)
  - [Vida e Carreira](game_design/life_&_career)
  - [Expressão Visual](game_design/visual_expression)

#### 🖥️ Análise e Desenvolvimento de Sistemas (ADS)

Relacionado ao meu tecnólogo em Análise e Desenvolvimento de Sistemas:

- [Análise e Desenvolvimento de Sistemas (ADS)](systems_analysis_&_development)
  - [Desenvolvimento Web](systems_analysis_&_development/web_development)

#### 🌐 Básico de Web

Conteúdo básico de web ou conteúdo genérico que pode ser usado em qualquer projeto web:

- [Web Básica](web_basics)
  - [HTML](web_basics/html)
  - [CSS](web_basics/css)
  - [Regex](web_basics/regex)

#### 🎨 Design de Web

Conteúdo relacionado a Design de Web:

- [Design de Web](web_design)
  - [Criando um Design](web_design/creating_a_design)
